...
public class BlockStateRandomizer {
    ...
    public BlockStateRandomizer(CompoundTag compoundTag) {
        this.defaultBlockState = Block.BLOCK_STATE_REGISTRY.byId(compoundTag.getInt("defaultBlockStateId").orElse(0));
        this.entries = new ArrayList<>();

        ListTag entriesTag = compoundTag.getList("entries").orElse(new ListTag());
        entriesTag.forEach(entryTag -> {
            CompoundTag entryCompoundTag = ((CompoundTag) entryTag);
            BlockState blockState = Block.BLOCK_STATE_REGISTRY.byId(entryCompoundTag.getInt("entryBlockStateId").orElse(0));
            float chance = entryCompoundTag.getFloat("entryChance").orElse(0.0f);
